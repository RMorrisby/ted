...
const (
	...
	ResultTableSelectAllNoSortingSQL = "SELECT suite.name, test.name, result.testrun, result.status, COALESCE(result.start_time, 'NULL'), result.end_time, result.ran_by, result.message, result.ted_status, result.ted_notes FROM " + ResultTable + " result LEFT JOIN " + SuiteTable + " suite ON result.suite_id = suite.id LEFT JOIN " + RegisteredTestTable + " test ON result.test_id = test.id"
